...
class RunningStdScaler:
    ...
    def __init__(self, state_dim, action_dim, device):
        ...
        self.sum_states = torch.zeros(self.state_dim, device=device)
        self.sum_actions = torch.zeros(self.action_dim, device=device)
    ...
    def update(self, states, actions):
        ...
        self.sum_states += torch.sum(states, dim=0).to(self.device)
        self.sum_actions += torch.sum(actions, dim=0).to(self.device)
    ...
    def mean(self):
        ...
        """
        if self.count == 0:
            return torch.zeros(self.state_dim, device=self.device), torch.zeros(self.action_dim, device=self.device)

        mean_states = self.sum_states / (self.count + self.epsilon)
        mean_actions = self.sum_actions / (self.count + self.epsilon)

        return mean_states, mean_actions

    def normalize_states(self, states, norm_mean=False):
        """
        ...
        if norm_mean:
            return (states - self.mean[0]) / self.std[0]
        else:
            return states / self.std[0]

    def normalize(self, states, actions = None, norm_mean=False):
        """
        ...
        if actions is None:
            ...
            if norm_mean:
                return (states - self.mean[0]) / self.std[0], (actions - self.mean[1]) / self.std[1]
            else:
    ...
    def load_state_dict(self, state_dict):
        ...
        self.sum_states = state_dict['sum_states'].to(self.device)
        self.sum_actions = state_dict['sum_actions'].to(self.device)
