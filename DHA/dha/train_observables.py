...
def main(cfg: DictConfig):
    ...
    if not training_done:
        ...
        stop_call = EarlyStopping(
            monitor="loss/val", mode="min", patience=max(cfg.system.early_stop_epochs, int(cfg.system.max_epochs * 0.1))
        )
        ...
        pl_model = LightLatentMarkovDynamics(
            lr=cfg.model.lr,
            batch_size=cfg.model.batch_size,
            run_hps=cfg.model,
        ...
        if training_successful:
            if not cfg.debug:  # Loading best model and test it
                if best_path.exists():
                    ...
                    log.warning("Best model not found, testing with latest model")
