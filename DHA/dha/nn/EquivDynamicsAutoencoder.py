...
class EquivDAE(DAE):
    ...
    def __init__(
        ...
        **dae_kwargs,
        ...
        self.obs_iso_reps, self.obs_iso_dims = isotypic_basis(
        ...
        super(EquivDAE, self).__init__(
            state_dim=self.state_type.size,
    ...
    def build_obs_fn(self, num_layers: int, **kwargs):
        return EMLP(in_type=self.state_type, out_type=self.obs_state_type, num_layers=num_layers, **kwargs)

    def build_inv_obs_fn(self, num_layers: int, **kwargs):
        return EMLP(in_type=self.obs_state_type, out_type=self.state_type, num_layers=num_layers, **kwargs)

    def build_obs_dyn_module(self) -> MarkovDynamics:
        return EquivLinearDynamics(
            state_type=self.obs_state_type, dt=self.dt, trainable=True, bias=self.enforce_constant_fn
        )
    ...
    for measurement in ["next_state"]:
        for i in range(n_steps):
            ...
            for s, g_s in zip(s_next, g_s_next):
                ...
                assert torch.allclose(absolute_error, torch.zeros(1), rtol=1e-5, atol=1e-5), (
                    f"t={i} - {absolute_error * 100:.2f}% error"
                )

    print("Done here it is your Equivariant Dynamics Autoencoder :)")
