...
class MarkovDynamics(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        assert state_dim is not None or state_rep is not None, "Either state_dim or state_rep must be provided"
    ...
    def forward(self, state: Tensor, next_state: Tensor, **kwargs) -> [Dict[str, Tensor]]:
        ...
        log.warning(
            f"MarkovDynamics.forward() is not implemented for {self.__class__.__name__}. "
    ...
    def post_process_state(self, state_traj: Tensor) -> Tensor:
        ...
        if self.state_inv_change_of_basis is not None:
            self.state_inv_change_of_basis = self.state_inv_change_of_basis.to(
                state_traj.device, dtype=state_traj.dtype
            )
            # Change basis to Isotypic basis.
        ...
        if len(state_traj.shape) == 2:
            return flat_to_batched_trajectory(state_traj, batch_size=self._batch_size, state_dim=self.state_dim)
        elif len(state_traj.shape) == 3:
            return state_traj
        else:
            raise ValueError(
                f"state_traj must be of shape (batch * time, state_dim) or "
                f"(batch, time, state_dim) tensor, got {state_traj.shape}"
            )

    def compute_loss_and_metrics(
        self, state_traj: Tensor, pred_state_traj: Tensor, **kwargs
    ...
    def check_state_traj_shape(
        self, state_dim: Optional[int] = None, time_horizon: Optional[int] = None, **state_trajectories
    ):
