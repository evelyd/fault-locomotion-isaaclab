        ...
        if constraint_matrix is not None:
            for i, constraint_id in enumerate(constraint_matrix):
                if constraint_id[2] != 0:  # Ensure z-coefficient isn't zero
                    z_coord = (
                        lambda x, y: (-constraint_id[0] * x - constraint_id[1] * y + constraint_offset[i])
                        / constraint_id[2]
                    )
                    ...
                    fig.add_trace(
                        go.Mesh3d(x=plane_corners[:, 0], y=plane_corners[:, 1], z=plane_corners[:, 2], opacity=0.2),
                        # col=col, row=row
                    )
...
def plot_trajectories(
    ...
    dt=1,
    dim_names=None,
    main_legend_label="traj",
    secondary_legend_label=None,
    shade_area=False,
    n_trajs_to_show=5,
    plot_error=True,
    col_shift=0,
    ...
    if fig is None:
        fig = make_subplots(
            rows=num_rows, cols=num_cols, subplot_titles=dim_names + (["MSE"] if plot_error else []), shared_xaxes=True
        )
        ...
        for traj_idx, color in enumerate(color_seq):
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    showlegend=show_legend and (dim == 0),
                ),
                row=row,
                col=col,
            )
    ...
    if plot_error:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines",
                fill="tozeroy",
                fillcolor="rgba(255, 0, 0, 0.1)",  # Added fill options here
    ...
    # Update layout
    fig.update_layout(title=title, plot_bgcolor="rgba(245, 245, 245, 1)", paper_bgcolor="rgba(245, 245, 245, 1)")
    return fig




def plot_two_panel_trajectories(
    state_trajs,
    obs_state_trajs,
    pred_obs_state_trajs=None,
    ...
    fig = make_subplots(
        rows=max(state_num_rows, obs_state_num_rows),
    ...
    fig = plot_trajectories(
        ...
        main_legend_label=main_legend_label,
        shade_area=True,
        secondary_legend_label=secondary_legend_label,
        n_trajs_to_show=n_trajs_to_show,
        plot_error=True,
    )
    ...
    fig = plot_trajectories(
        obs_state_trajs,
        pred_obs_state_trajs,
        fig=fig,
        dt=dt,
        dim_names=obs_state_dim_names,
        main_legend_label=main_legend_label,
        shade_area=True,
        secondary_legend_label=secondary_legend_label,
        n_trajs_to_show=n_trajs_to_show,
