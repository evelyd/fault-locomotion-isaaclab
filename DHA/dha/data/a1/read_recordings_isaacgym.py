...
def convert_cyberdog2_isaacgym_recordings(data_paths: list):
    ...
    for data_path in data_paths:
        ...
        all_actions.append(np.array([traj['action'] for traj in data]))
    print(f"Loaded {len(all_obs)} files. Obs shape: {all_obs[0].shape}, action shape: {all_actions[0].shape}")
    ...
    num_timesteps = 1000
    num_envs = 32
    state_reshaped = state.reshape(len(data_paths), num_timesteps, num_envs, state.shape[-1])
    reordered_state = state_reshaped.transpose(0, 2, 1, 3)
    state = reordered_state.reshape(len(data_paths) * num_envs * num_timesteps, state.shape[-1])

    action_reshaped = action.reshape(len(data_paths), num_timesteps, num_envs, action.shape[-1])
    reordered_action = action_reshaped.transpose(0, 2, 1, 3)
    action = reordered_action.reshape(len(data_paths) * num_envs * num_timesteps, action.shape[-1])
    ...
    rep_xy = group_rep_from_gens(G, rep_H={h: rep_Rd(h)[:2, :2].reshape((2, 2)) for h in G.elements if h != G.identity}) # for the lin velocity commands
    rep_xy.name = "base_xy"
    rep_euler_z = group_rep_from_gens(G, rep_H={h: rep_euler_xyz(h)[2, 2].reshape((1, 1)) for h in G.elements if h != G.identity}) # for ang vel command
