...
class Console():
    def __init__(self, controller_node):
        ...
        # Autocomplete setup
        self.commands = [
            "help", "ictp", "goUp", "goDown", "activate", "ictp", "setKp", "setKd", "setFailureMode"
        ]
        ...
        while True:
            ...
            try:
                if(input_string == "goUp"):
                    ...
                    if(temp != ""):
                        if(temp == "1"): #None
                            self.controller_node.locomotion_policy._per_leg_joint_status = np.ones((4,3), dtype=np.float32)
                        elif(temp == "2"): #RL-RR failure
                            self.controller_node.locomotion_policy._per_leg_joint_status[2, :] = 0.0
                            self.controller_node.locomotion_policy._per_leg_joint_status[3, :] = 0.0
                        elif(temp == "3"): #FL failure
                            self.controller_node.locomotion_policy._per_leg_joint_status[0, :] = 0.0
                        elif(temp == "4"): #FR failure
                            self.controller_node.locomotion_policy._per_leg_joint_status[1, :] = 0.0
                        elif(temp == "5"): #RL failure
                            self.controller_node.locomotion_policy._per_leg_joint_status[2, :] = 0.0
                        elif(temp == "6"): #RR failure
                            self.controller_node.locomotion_policy._per_leg_joint_status[3, :] = 0.0
    ...
    def print_all_commands(self):
        ...
        print("setFailureMode: Set the failure mode for testing the recovery capabilities of the policy")
